...
function parallaxSlider(options) {
    var $ = jQuery;
    this.setting = {
        displaymode: {type: 'auto', pause: 2000, stoponclick: false, cycles: 2, pauseonmouseover: true},
        delaybtwdesc: 500,
...
parallaxSlider.prototype = {
    slide: function (nextslide, dir) { //possible values for dir: "left", "right", "up", or "down"
        ...
        var nextslide_initialpos = setting.dimensions[(dir == "right" || dir == "left") ? 0 : 1]
        ...
        var $nextslide = this.$imageslides.eq(nextslide).show().css(
        ...
        //show upcoming slide: [.75,.03,0,.97] is easing "function". See http://cubic-bezier.com
        $nextslide.stop().velocity(createobj([posprop, 0]), setting.slideduration, [.75, .03, 0, .97], function () {
        ...
        $nextdescs.each(function (i) { //show upcoming slide's descs
            ...
            $desc.stop().css(createobj([posprop, nextslide_initialpos])).velocity(
                createobj([posprop, 0]), [.75, .03, 0, .97],
                $desc.data('duration')
            )
        });

        //unshow current slide
        $curslide.css({zIndex: 0}).removeClass(setting.activeslideclass)
            .stop().velocity(
            createobj([posprop, dir == 'left' || dir == 'up' ? -100 : 100]),
        ...
        //unshow current slide's descs: [.52,.25,.94,.51] is easing "function". See http://cubic-bezier.com
        $curdescs.each(function (i) {
            var $desc = jQuery(this);
            $desc.stop().velocity(
                createobj([posprop, dir == 'left' || dir == 'up' ? '-100%' : '100%']),
                [.52, .25, .94, .51],
                $desc.data('duration') * .5
            );
        });
    ...
    rotate: function () {
        ...
        //go to next slide, either to the left or upwards depending on setting.orientation setting
    ...
    init: function ($) {
        ...
        //make sure curslide index is within bounds
        ...
        this.$imageslides.each(function (i) {
            var $descs = $(this).find('div.desc').css({
                position: 'absolute',
                left: 0,
                top: 0,
                width: '100%',
        ...
        var $controls = $('<img class="navbutton" src="'
            + controlpaths[1] + '" data-dir="forth" style="position:absolute; z-index:5; cursor:pointer; '
            + (orientation == 'v' ? 'bottom:8px; left:46%' : 'top:46%; right:8px;')
            + '" />'
            + '<img class="navbutton" src="' + controlpaths[0]
            + '" data-dir="back" style="position:absolute;z-index:5; cursor:pointer; '
            + (orientation == 'v' ? 'top:8px; left:45%' : 'top:45%; left:8px;')
            + '" />'
        ...
        swipeOptions.swipeStatus = function (event, phase, direction, distance) {
            // cancel A action when finger makes contact with element
            if (phase == 'start' && event.target.tagName == 'A') {
                evtparent.onclick = function () {
                    return false
                }
            }

            // if swipe action canceled (so no proper swipe), enable A action
            if (phase == 'cancel' && event.target.tagName == 'A') {
                evtparent.onclick = function () {
                    return true
                }
            }
            if (phase == 'end') {
                ...
                if ((setting.orientation == 'h' && /(left)|(right)/i.test(direction))
                    || (setting.orientation == 'v' && /(up)|(down)/i.test(direction))) {
...
parallaxSlider.routines = {
    getSlideHTML: function (setting, imgref, w, h, posprop) {
        ...
        return '<div class="slide" style="background-image:url('
            + imgref[0] + '); position:absolute;'
            + posstr + ';width:' + w + '; height:' + h + '; z-index: 0">'
