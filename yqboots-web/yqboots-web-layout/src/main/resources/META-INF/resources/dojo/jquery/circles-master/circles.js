...
(function () {
    ...
    Circles.prototype = {
        ...
        _confirmAnimation: function (duration) {
            ...
            var step = duration / this._interval, pathFactor = this._percentage / step, numberFactor = this._number / step;
        ...
        _generateText: function () {
            var html = '<div class="' + this._textWrpClass + '" style="position:absolute; top:0; left:0; text-align:center; width:100%;'
                + ' font-size:' + this._radius * .5 + 'px; height:'
        ...
        _calculateNumber: function (number) {
            var parts = (number + '').split('.'), html = '<span class="' + this._numberClass + '">' + parts[0];
            if (parts.length > 1) {
        ...
        _generatePath: function (percentage, open, color) {
            return '<path fill="transparent" stroke="' + color + '" stroke-width="'
                + this._strokeWidth + '" d="'
                + this._calculatePath(percentage, open) + '"/>';
        },
        ...
        _arc: function (end, open) {
            var endAdjusted = end - 0.001, longArc = end - this._startPrecise < Math.PI ? 0 : 1;
